...
static const char DASHBOARD_HTML[] =
    ...
    ".app{position:absolute;left:0;top:0;width:1500px!important;max-width:none!important;margin:0!important;transform-origin:0 0;will-change:transform}\n"
    ...
    "</svg>\n"
    "<div id='stage'><div id='app' class='app'>\n"
    "<header class='hero'>\n"
    ...
    "let busy=false,settings={},healthTimer=null,countTimer=null,baseline='',fitRaf=0;\n"
    "function fitViewport(){cancelAnimationFrame(fitRaf);fitRaf=requestAnimationFrame(()=>{const a=$('app'),pad=10;a.style.transform='none';const w=a.offsetWidth,h=a.scrollHeight;const s=Math.min((window.innerWidth-pad*2)/w,(window.innerHeight-pad*2)/h);const scale=Math.max(.05,s);const x=Math.max(pad,(window.innerWidth-w*scale)/2),y=Math.max(pad,(window.innerHeight-h*scale)/2);a.style.left=x+'px';a.style.top=y+'px';a.style.transform='scale('+scale+')'})}\n"
    ...
    " $('pending').classList.toggle('show',!!j.wifi_pending);$('off_note').textContent=j.modem_off_s||20;baseline=JSON.stringify(payload());$('save').disabled=true;fitViewport()\n"
    "}\n"
    ...
    "  $('pending').classList.toggle('show',!!h.wifi_pending);if(h.modem_off_s)$('off_note').textContent=h.modem_off_s;fitViewport()\n"
    " }catch(e){}\n"
    "}\n"
    "function payload(){\n"
    ...
    "window.addEventListener('resize',fitViewport,{passive:true});window.addEventListener('orientationchange',fitViewport);fitViewport();Promise.all([loadSettings(),loadHealth()]).catch(e=>toast('Falha ao carregar dashboard','error'));healthTimer=setInterval(loadHealth,10000);\n"
    "</script>\n"
    "</body>\n"
    "</html>\n";
