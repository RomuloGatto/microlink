...
static const char DASHBOARD_HTML[] =
    ...
    "let busy=false,settings={},healthTimer=null,countTimer=null;\n"
    ...
    " const r=await fetch('/api/settings',{cache:'no-store'});if(!r.ok)throw new Error('settings '+r.status);const j=await r.json();settings=j;\n"
    ...
    " $('pending').classList.toggle('show',!!j.wifi_pending);$('off_note').textContent=j.modem_off_s||20\n"
    "}\n"
    ...
    " try{const r=await fetch('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload())});const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.error||'Falha ao salvar');toast('Configuracoes salvas','success');showApplying(!!j.wifi_changed)}\n"
    ...
    " try{const r=await fetch('/reboot',{method:'POST'});if(!r.ok)throw new Error(await r.text());\n"
    ...
    "$('form').addEventListener('submit',save);$('reboot').onclick=showConfirm;$('cancel').onclick=modalClose;$('confirm').onclick=doReboot;$('overlay').onclick=e=>{if(e.target===$('overlay'))modalClose()};document.addEventListener('keydown',e=>{if(e.key==='Escape')modalClose()});\n"
