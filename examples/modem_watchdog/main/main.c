...
static const char DASHBOARD_HTML[] =
    ...
    "<style>\n"
    ":root{--bg:#07111e;--panel:#0d1a2b;--panel2:#0a1625;--line:#203650;--line2:#294765;--text:#edf4ff;--muted:#93a8c4;--blue:#438cff;--green:#27c45b;--red:#f14955;--amber:#f4b942;--shadow:0 18px 48px #0007}\n"
    "*{box-sizing:border-box}html,body{margin:0;min-height:100%;background:radial-gradient(circle at 16% 0,#0d2641 0,transparent 28%),linear-gradient(180deg,#07111e,#06101b 70%);color:var(--text);font:15px -apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif}\n"
    "button,input{font:inherit}button{cursor:pointer}.app{width:min(1420px,calc(100% - 36px));margin:24px auto 42px}\n"
    ".hero{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;margin-bottom:20px}.brand{display:flex;align-items:center;gap:16px}.brandmark{width:54px;height:54px;color:#4595ff;display:grid;place-items:center}.brandmark svg{width:52px;height:52px}.hero h1{font-size:30px;line-height:1.05;margin:0;font-weight:800;letter-spacing:-.02em}.hero p{margin:7px 0 0;color:var(--muted);font-size:16px}\n"
    ".badge{display:none;align-items:center;gap:9px;padding:13px 20px;border-radius:999px;background:linear-gradient(180deg,#f04d61,#db2d48);font-weight:800;box-shadow:0 10px 28px #e23b5040}.badge.show{display:flex}.badge svg{width:20px;height:20px}\n"
    ".notice{display:none;margin:-5px 0 16px;padding:12px 15px;border:1px solid #715c22;border-radius:13px;background:#2a2413;color:#f4d579}.notice.show{display:block}\n"
    ".stats{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:12px;margin-bottom:18px}.stat,.panel{background:linear-gradient(180deg,#102036cc,#0a1727ee);border:1px solid var(--line);box-shadow:var(--shadow)}.stat{min-height:92px;border-radius:15px;padding:15px 16px;display:flex;gap:13px;align-items:center}.stat-icon,.panel-icon{color:#6ca9ff;flex:0 0 auto}.stat-icon{width:38px;height:38px;display:grid;place-items:center}.stat-icon svg{width:29px;height:29px}.stat-label{color:#a9bad1;font-size:13px;margin-bottom:4px}.stat-value{font-weight:800;font-size:17px;line-height:1.18;word-break:break-word}.dot{display:inline-block;width:10px;height:10px;border-radius:50%;margin-right:8px;vertical-align:-1px}.dot.ok{background:var(--green);box-shadow:0 0 10px #27c45b70}.dot.bad{background:var(--red);box-shadow:0 0 10px #f1495570}.state-warn{color:#ffd36a}\n"
    ".layout{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px}.stack{display:grid;gap:16px;align-content:start}.panel{border-radius:18px;padding:22px}.panel-head{display:flex;gap:14px;align-items:flex-start;margin-bottom:18px}.panel-icon{width:34px;height:34px}.panel-icon svg{width:30px;height:30px}.panel h2{font-size:18px;margin:0 0 4px;font-weight:800}.panel-sub{margin:0;color:var(--muted);font-size:13px}\n"
    ".fields{display:grid;grid-template-columns:1fr 1fr;gap:14px}.field{min-width:0}.field.full{grid-column:1/-1}.field label{display:block;margin:0 0 7px;color:#c7d6e9;font-size:13px}.hint{color:#7f96b2;font-size:12px;margin-top:6px;line-height:1.35}.input-wrap{position:relative}.input{width:100%;height:46px;border-radius:11px;border:1px solid var(--line2);background:#081421;color:#f3f7fd;padding:0 14px;outline:none;transition:.18s}.input:focus{border-color:#4d8fe6;box-shadow:0 0 0 3px #438cff1d}.input::placeholder{color:#6f8199}.input.has-eye{padding-right:46px}.eye{position:absolute;right:6px;top:5px;width:36px;height:36px;border:0;background:transparent;color:#7895ba;border-radius:9px;display:grid;place-items:center}.eye:hover{background:#ffffff09;color:#a9c8ed}.eye svg{width:19px;height:19px}\n"
    ".switch-row{display:flex;align-items:center;justify-content:space-between;gap:18px;margin-bottom:14px}.switch-title{font-weight:650}.switch-desc{color:var(--muted);font-size:12px;margin-top:3px}.switch{position:relative;width:55px;height:30px;flex:0 0 55px}.switch input{display:none}.switch span{position:absolute;inset:0;border-radius:99px;background:#314156;border:1px solid #41556f;transition:.18s}.switch span:after{content:'';position:absolute;width:22px;height:22px;left:3px;top:3px;background:white;border-radius:50%;transition:.18s;box-shadow:0 3px 8px #0005}.switch input:checked+span{background:#20aa4b;border-color:#30c65d}.switch input:checked+span:after{transform:translateX(25px)}.switch input:disabled+span{opacity:.45;cursor:not-allowed}\n"
    ".watch-grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:14px}.footer{display:grid;grid-template-columns:1fr auto;align-items:end;gap:22px;margin-top:16px}.health-link{color:#65adff;text-decoration:none;font-size:14px}.actions{display:grid;grid-template-columns:minmax(250px,350px) minmax(250px,350px);gap:16px}.btn{height:50px;border-radius:12px;border:0;color:white;font-weight:800}.btn:disabled{opacity:.55;cursor:wait}.btn-primary{background:linear-gradient(180deg,#4c96ff,#2f79f6);box-shadow:0 10px 24px #387fe43d}.btn-danger{background:linear-gradient(180deg,#fb5960,#ed3949);box-shadow:0 10px 24px #e8404c35}.footer-note{text-align:right;color:#8095b0;font-size:11px;margin-top:6px}\n"
    ".toast{position:fixed;right:22px;bottom:22px;z-index:80;max-width:420px;padding:13px 16px;border-radius:12px;background:#0d1a2b;border:1px solid var(--line2);box-shadow:var(--shadow);opacity:0;transform:translateY(10px);pointer-events:none;transition:.18s}.toast.show{opacity:1;transform:none}.toast.error{border-color:#7a3140;color:#ffb3bb}.toast.success{border-color:#27683e;color:#b7f7c8}\n"
    ".overlay{position:fixed;inset:0;background:#020814c7;backdrop-filter:blur(5px);display:none;align-items:center;justify-content:center;padding:20px;z-index:100}.overlay.show{display:flex}.modal{width:min(460px,100%);border-radius:20px;background:linear-gradient(180deg,#12233a,#0a1727);border:1px solid #294765;box-shadow:0 30px 90px #000b;padding:22px}.modal-top{display:flex;gap:13px;align-items:flex-start}.modal-symbol{width:42px;height:42px;border-radius:13px;background:#f1495515;color:#ff6872;display:grid;place-items:center;flex:0 0 auto}.modal-symbol.blue{background:#438cff15;color:#65a7ff}.modal-symbol svg{width:22px;height:22px}.modal h3{margin:1px 0 5px;font-size:19px}.modal p{margin:0;color:var(--muted);line-height:1.45;font-size:13px}.modal-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:22px}.modal-btn{height:42px;padding:0 16px;border-radius:10px;border:1px solid #31455d;background:#17263a;color:white;font-weight:700}.modal-btn.danger{border:0;background:#ed3f4c}.countdown{display:none;margin-top:18px}.countdown.show{display:block}.count-num{font-size:48px;font-weight:900;letter-spacing:-.04em;text-align:center}.count-label{text-align:center;color:var(--muted);font-size:12px}.progress{height:8px;margin-top:15px;border-radius:99px;background:#ffffff0f;overflow:hidden}.progress>div{height:100%;width:0;background:#438cff;transition:width .9s linear}\n"
    "@media(min-width:861px){.app{width:min(1480px,calc(100% - 24px));margin:12px auto 16px}.hero{margin-bottom:10px;gap:14px}.brand{gap:12px}.brandmark{width:42px;height:42px}.brandmark svg{width:40px;height:40px}.hero h1{font-size:24px}.hero p{font-size:13px;margin-top:3px}.badge{padding:9px 14px;font-size:12px}.badge svg{width:17px;height:17px}.notice{margin:-2px 0 9px;padding:8px 11px;font-size:12px}.stats{gap:9px;margin-bottom:10px}.stat{min-height:68px;padding:9px 11px;gap:9px;border-radius:12px}.stat-icon{width:29px;height:29px}.stat-icon svg{width:23px;height:23px}.stat-label{font-size:11px;margin-bottom:2px}.stat-value{font-size:14px}.dot{width:8px;height:8px;margin-right:6px}.layout{gap:10px;margin-bottom:10px}.stack{gap:10px}.panel{padding:13px 15px;border-radius:14px}.panel-head{gap:9px;margin-bottom:9px}.panel-icon{width:27px;height:27px}.panel-icon svg{width:24px;height:24px}.panel h2{font-size:15px;margin-bottom:2px}.panel-sub{font-size:11px}.fields{gap:9px 10px}.field label{font-size:11px;margin-bottom:4px}.hint{font-size:10px;margin-top:3px;line-height:1.25}.input{height:36px;border-radius:9px;padding:0 11px;font-size:13px}.input.has-eye{padding-right:40px}.eye{right:4px;top:3px;width:30px;height:30px}.eye svg{width:16px;height:16px}.switch-row{gap:12px;margin-bottom:9px}.switch-title{font-size:12px}.switch-desc{font-size:10px;margin-top:2px}.switch{width:46px;height:25px;flex-basis:46px}.switch span:after{width:19px;height:19px;left:2px;top:2px}.switch input:checked+span:after{transform:translateX(22px)}.watch-grid{gap:9px}.footer{gap:14px;margin-top:9px}.health-link{font-size:12px}.actions{grid-template-columns:minmax(210px,290px) minmax(210px,290px);gap:10px}.btn{height:42px;border-radius:10px;font-size:13px}.footer-note{font-size:9px;margin-top:4px}.toast{right:14px;bottom:14px}.modal{padding:18px}.modal-actions{margin-top:16px}}\n"
    "@media(min-width:861px) and (max-height:780px){.app{margin:7px auto 9px}.hero{margin-bottom:6px}.brandmark{width:36px;height:36px}.brandmark svg{width:34px;height:34px}.hero h1{font-size:21px}.hero p{font-size:11px}.stats{margin-bottom:7px;gap:7px}.stat{min-height:58px;padding:7px 9px}.stat-icon{width:25px;height:25px}.stat-icon svg{width:20px;height:20px}.stat-label{font-size:10px}.stat-value{font-size:13px}.layout{gap:8px;margin-bottom:8px}.stack{gap:8px}.panel{padding:10px 12px}.panel-head{margin-bottom:6px}.panel-icon{width:24px;height:24px}.panel-icon svg{width:21px;height:21px}.panel h2{font-size:14px}.panel-sub{font-size:10px}.fields{gap:7px 9px}.field label{font-size:10px;margin-bottom:3px}.hint{font-size:9px;margin-top:2px}.input{height:32px;font-size:12px}.eye{top:1px}.switch-row{margin-bottom:6px}.switch-desc{display:none}.watch-grid{gap:7px}.footer{margin-top:7px}.btn{height:38px}.footer-note{margin-top:2px}}\n"
    "@media(max-width:980px){.stats{grid-template-columns:repeat(3,1fr)}.watch-grid{grid-template-columns:repeat(3,1fr)}}@media(max-width:860px){.layout{grid-template-columns:1fr}.actions{grid-template-columns:1fr 1fr}.footer{grid-template-columns:1fr}.footer-note{text-align:left}}@media(max-width:640px){.app{width:calc(100% - 20px);margin-top:15px}.hero{align-items:center}.hero h1{font-size:23px}.hero p{font-size:13px}.brandmark{width:44px;height:44px}.brandmark svg{width:42px;height:42px}.badge{padding:10px 12px;font-size:12px}.stats{grid-template-columns:1fr 1fr}.fields,.watch-grid{grid-template-columns:1fr}.field.full{grid-column:auto}.panel{padding:17px}.actions{grid-template-columns:1fr}.footer-note{text-align:center}}@media(max-width:420px){.stats{grid-template-columns:1fr}.hero{align-items:flex-start}.badge{position:absolute;right:10px;top:12px}}\n"
    "</style>\n"
    "</head>\n"
    "<body>\n"
    ...
    "<div class='field' style='margin-bottom:14px'><label>Hostname</label><input id='ctrl_host' class='input' maxlength='63' required></div>\n"
    ...
    "<div class='field'><label>Internet check interval (s)</label><input id='check_interval_s' class='input' type='number' min='5' max='3600'></div>\n"
    ...
    "<div class='field'><label>Max automatic reboots</label><input id='max_auto_reboots' class='input' type='number' min='1' max='20'></div>\n"
    "<div class='field'><label>Reboot limit window (s)</label><input id='reboot_window_s' class='input' type='number' min='60' max='86400'></div>\n"
    "</div>\n"
    "</section>\n"
    "<div class='footer'>\n"
    "<a class='health-link' href='/health' target='_blank'>Ver JSON de health &#8594;</a>\n"
    "<div><div class='actions'><button id='save' class='btn btn-primary' type='submit'>Salvar configuracoes</button><button id='reboot' class='btn btn-danger' type='button'>Reiniciar modem</button></div><div class='footer-note'>A energia sera cortada por <span id='off_note'>20</span> segundos &bull; status atualiza a cada 10s</div></div>\n"
    "</div>\n"
    "</form>\n"
    "</div>\n"
    ...
    "  $('st_ts').innerHTML=dot(!!h.tailscale_connected,h.tailscale_connected?'conectado':'desconectado');\n"
